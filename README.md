...
```bash
./run.sh game/pong.ch8
```

## 5. Instruction set

I made an alternative to the assembly language shown in the Cowgod's reference. It's based on CHIP-8's instruction set just for debugging and classify opcodes, and that is what is shown in the memory viewer.
...
There is no labels, named constants, expresions or anything else (maybe in the future?). In fact, there is no assembler at all (only a disassembler for the memory viewer).
